...
impl TaskLifecycle {
    ...
    pub async fn create_task(
        ...
        name: &str,
        owner: Option<&str>,
    ) -> ArmorResult<crate::task::registry::Task> {
        self.registry
            .create(id, name, owner)
            .await
            .map_err(|e| ArmorError::Database(e.to_string()))?;

        if let Err(e) = self
            .registry
            .add_event(id, "created", &format!("Task '{}' created", name))
            .await
        {
            tracing::warn!(
                "AUDIT WRITE FAILED for task {} (created): {} — audit trail is incomplete",
                id,
                e
            );
        }
    ...
    pub async fn cancel_task(&self, id: &str) -> ArmorResult<crate::task::registry::Task> {
        ...
        if let Err(e) = self
            .registry
            .add_event(id, "cancelled", "Task cancelled")
            .await
        {
            tracing::warn!(
                "AUDIT WRITE FAILED for task {} (cancelled): {} — audit trail is incomplete",
                id,
                e
            );
        }
        ...
        if existed {
            if let Err(e) = self
                ...
                .add_event(
                    id,
                    "task_deleted",
                    "Task deleted (events retained for audit)",
                )
                .await
            {
                tracing::warn!(
                    "AUDIT WRITE FAILED for task {} (task_deleted): {} — audit trail is incomplete",
                    id,
                    e
                );
            }
