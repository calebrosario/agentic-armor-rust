...
#[test]
fn test_error_codes() {
    assert_eq!(ArmorError::TaskNotFound("x".into()).code(), "TASK_NOT_FOUND");
    assert_eq!(ArmorError::ForbiddenMount("x".into()).code(), "FORBIDDEN_MOUNT");
    assert_eq!(ArmorError::HostNetworkDenied.code(), "HOST_NETWORK_DENIED");
    assert_eq!(ArmorError::InvalidNetworkMode("x".into()).code(), "INVALID_NETWORK_MODE");
    assert_eq!(ArmorError::InvalidMountConfig("x".into()).code(), "INVALID_MOUNT_CONFIG");
    assert_eq!(ArmorError::InvalidPath("x".into()).code(), "INVALID_PATH");
    assert_eq!(ArmorError::PathRestricted.code(), "PATH_RESTRICTED");
    assert_eq!(ArmorError::ContainerCreateFailed("x".into()).code(), "CONTAINER_CREATE_FAILED");
    assert_eq!(ArmorError::DockerConnectionFailed("x".into()).code(), "DOCKER_CONNECTION_FAILED");
